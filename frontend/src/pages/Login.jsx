...
import { loginUser, registerUser, loginWithGoogle, sendOTP, verifyOTP } from '../services/api';
...
export default function Login() {
  ...
  const handleSendOTP = async (e) => {
    ...
    setSuccess(null);
    try {
      await sendOTP({ email: formData.email, purpose: 'login' });
      setOtpSent(true);
      setTimer(60);
      setCanResend(false);
    ...
    <div className="min-h-screen flex items-center justify-center p-4 relative overflow-hidden bg-black">
      ...
      <div className="w-full max-w-sm relative z-10">
        <div className="flex flex-col items-center mb-8">
          <img src="/logo.png" alt="SubTracker Logo" className="w-16 h-16 object-contain mb-4" />
          <h1 className="text-display-md text-text-primary tracking-tight">SubTracker</h1>
          <h1 className="text-display-sm text-text-primary text-center">
            {isOtpMode ? 'Passwordless Login' : isLogin ? 'Welcome back' : 'Create an account'}
          </h1>
          <p className="text-caption text-text-secondary mt-2 text-center">
            {isOtpMode ? 'We will send a code to your email.' : isLogin ? 'Enter your details to access your dashboard.' : 'Start tracking your subscriptions today.'}
          </p>
        </div>
        ...
        <div className="card p-6 md:p-8 relative bg-surface-1/50 backdrop-blur-3xl border-line">
          ...
          {!isOtpMode ? (
            ...
            <div className="space-y-4">
              <div>
                <label className="block text-caption font-medium text-text-secondary mb-1.5">Email address</label>
                <input required disabled={otpSent} type="email" value={formData.email} onChange={e => setFormData({...formData, email: e.target.value})} className="w-full bg-surface-2 border border-line rounded-lg px-3 py-2 text-text-primary outline-none focus:border-accent transition-colors disabled:opacity-50" placeholder="hello@linear.app" />
              </div>
              
              {otpSent && (
                <div>
                  <label className="block text-caption font-medium text-text-secondary mb-1.5">6-Digit Code</label>
                  <input required type="text" maxLength={6} value={formData.otp} onChange={e => setFormData({...formData, otp: e.target.value})} className="w-full bg-surface-2 border border-line rounded-lg px-3 py-2 text-text-primary outline-none focus:border-accent transition-colors tracking-widest text-center text-lg font-bold" placeholder="------" />
                </div>
              )}

              <div className="pt-2 flex flex-col gap-4">
                {!otpSent ? (
                  <button disabled={loading || !formData.email} onClick={handleSendOTP} className="btn-primary w-full disabled:opacity-50 flex justify-center items-center gap-2">
                    <Mail className="w-4 h-4" />
                    {loading ? 'Sending...' : 'Send Magic Link / OTP'}
                  </button>
                ) : (
                  <>
                    <button disabled={loading} onClick={handleVerifyOTP} className="btn-primary w-full disabled:opacity-50">
                      {loading ? 'Verifying...' : 'Verify & Login'}
                    </button>
                    {canResend ? (
                      <button type="button" disabled={loading} onClick={handleSendOTP} className="text-caption text-text-secondary hover:text-orange-500 transition-colors">
          ...
          <div className="flex flex-col gap-3">
            <button 
              type="button" 
              onClick={() => { setIsOtpMode(!isOtpMode); setError(null); setSuccess(null); }} 
            ...
            >
              {isOtpMode ? 'Use Password Instead' : 'Login with Email OTP'}
            </button>
            <button 
              type="button" 
              onClick={() => googleLogin()} 
